...
const ProductPage = ({ handleNav, handleProductView, addToEnquiry, searchQuery = '', initialCategory = 'All', enquiryCart = [] }) => {
    ...
    // Reset filtering when search query changes
    useEffect(() => {
        if (searchQuery) {
    ...
    // Handle incoming initial category (like clicking a nav link)
    useEffect(() => {
        if (initialCategory) {
        ...
        <div className="animate-in fade-in">
            <div className="bg-[#f8fafc] py-12 border-b border-slate-200">
                <div className="max-w-7xl mx-auto px-4 text-center">
                    <h1 className="text-4xl font-bold text-[#0B2C4D] mb-4">
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-6 pb-2 border-b border-slate-200">
                            ...
                            <span className="text-sm text-slate-500">
                                Showing {currentItems.length > 0 ? itemsOfFirstItem + 1 : 0}-{Math.min(indexOfLastItem, filteredProducts.length)} of {filteredProducts.length} results
                            </span>
                        </div>
